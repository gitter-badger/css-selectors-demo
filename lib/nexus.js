...
nxt.ClassRenderer = {
	...
	render: function (data, domContext) {
		var classes = data.name.split(' ');

		if (domContext.content) {
			domContext.content.forEach(function (className) {
				if (classes.indexOf(className) === -1) {
					domContext.container.classList.remove(className);
				}
			});
		}

		classes.forEach(function (className) {
			domContext.container.classList.add(className);
		});
		return classes;
	},
